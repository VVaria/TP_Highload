...
Просмотренные хештеги и пользователей будем хранить в таблице Redis для быстрого доступа к этим данным. Обновляться хештеги будут при каждом посещении пользователя. И при частом посещении повышаться их рейтинг важности. В этом будет помогать Kafka, которая будет посылать сообщения для переформировывания рекомендаций пользователя, которые находятся в базе Statistics.
...
# Схема проекта
![image](https://user-images.githubusercontent.com/34479597/147254209-694f98fc-82b9-4207-ad34-221b14675917.png)
...
Пароли будем хранить в зашифрованном виде, и исопльзовать рекомендации OWASP(sha3 + bcrypt).

Размер password = 60 Байт

Размер datetime = 8 Байт

Размер Users

```
id + username + email + password = 16 + 70 + 70 + 60 = 216 Байт
216 Байт * 2,5 млн = 0,5 Гб ежемесячно (в настоящий момент: 216 * 490 = 98 Гб)
```

Размер Blog

```
id + post + hashtag = 16 * 3 = 48 Байт
48 Байт * 7,2 млн = 0,3 Гб ежемесячно
```

Размер Posts

```
id + text + content + time + user = 16 + 4000 + 10 * 150 + 8 + 16 = 5540 Байт
5540 Байт * 22 млн = 113 Гб ежемесячно (всего 5540 * 180 млрд = 906 ТБ)
```

Размер Statistics

```
id + user + hashtag + interested_user + rate = 16 * 4 + 4 = 68 Байт
68 Байт * 320 млн = 20 Гб ежемесячно (всего 68 * 490 млн = 31 ГБ)
```
...
:---------: | :-----------: | :-----------:| :-------: | :----------:
64          |   256         |     512      |   2 х 5   | 114 + 114 (резерв)  

100 баз -> посты - 96, блоги - 12, пользователи - 3, статистика - 3
...
Kafka используется для сбора действий пользователя, статистики. Это нужно для построения рекомендаций. Kafka будет послыать сообщения для переформировывания рекомендаций пользователя основываясь на нескольких действиях, а не на одном.
...
7. https://tproger.ru/video/video-osnovy-apache-kafka/
