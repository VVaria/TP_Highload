...
# Логическая схема
![image](https://user-images.githubusercontent.com/34479597/146657092-b10a7e1d-ee00-47be-9227-ee0dc6dc35b7.png)

# Физическая схема
Базы пользователей, постов и блога будут храниться в реляционной СУБД PostgreSQL. В данном случае это является наиболее простым и выгодным решением. Также будет настроены шардинг и репликация для снижения нагрузки на взаимодействие с данными и поддержания отказоустойчивости. Для чтения будем использовать как раз реплики по три штуки на каждую таблицу.  Внутренним сервисам необходим доступ к потоку всех событий в системе (создание, редактирование и удаление постов), для чего можно создать внутреннюю шину сообщений c использованием Kafka. Это разгрузит поток запросов, который достаточно большой и запросы будут браться в порядке очереди. Для Users.id, Posts.id, Posts.user делаем hash-индексацию для быстрого поиска и сравнения. Шардирование 
...
Помимо балансировки на уровне DNS мы будем использовать балансировку Nginx. Nginx будет иметь огромную нагрузку из-за большого количества данных, поэтому нам нужно будет побольше RAM и CPU.
...
В производственной среде серверы ZooKeeper будут развернуты на несколько узлов. Нечетное количество серверов позволяет ZooKeeper проводить выборы большинства для лидерство.
Также мы используем kafka для общения между микросервисами. Нам понадобится много памяти, для того, чтобы хранить информацию о состоянии пользователя, а по скольку соеденений много потребуется RAM и CPU. Кластер состоит из 3 узлов поддержит пропускную способность ~800 сообщений в секунду.
